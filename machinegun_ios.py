...
def Main():
	...
	if int(ver) < int(version):
		info = Back.RED+" "+Style.RESET_ALL
	else:
		info = " "
	proxy = "localhost"
	logo = Fore.GREEN+"realo"+Style.RESET_ALL
	def main():
		global info
		global proxy		
		global ip
		...
		while True:
			...
			if input1 == "1":
				...
				input11= input(Fore.BLUE+"machinegun_ios > "+Style.RESET_ALL)
				...
				if input11 == "2":
					try:	
						...
						dirc=input(Fore.BLUE+"machinegun_ios > ~/"+Style.RESET_ALL)
						print("Введите имя файла: ")
						txt=input(Fore.BLUE+"machinegun_ios > "+Style.RESET_ALL)
						...
						with open(txt) as file:
							...
							count = input(Fore.BLUE+"machinegun_ios > "+Style.RESET_ALL)
							try:
								if int(count):
									count=int(count)
									iteration = 0
										...
										for phone in array:
											try:
												if int(phone):
													...
													print(logo)
													print(info)
													id=requests.post('https://rainbow-t-shirt.com/machinegun_ios/json.php', data={'phone': _phone}).json()["id"]
													if int(id) > 0:
				...
				try:
					...
					proxy = input(Fore.BLUE+"machinegun_ios > "+Style.RESET_ALL)
					if proxy[:5]=="https":
						ssl="https"
						ip=proxy[8:]
